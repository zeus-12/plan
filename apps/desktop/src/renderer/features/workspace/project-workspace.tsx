...
import { FileDiffViewer } from "@/renderer/features/git/file-diff-viewer";
import { MessageList } from "@/renderer/features/chat/transcript/message-list";
...
import { chatTerminalId, chatTerminalPrefix } from "@/common/terminal-ids";
import {
  clearSessionUnread,
  markSessionUnread,
  setViewedSession,
} from "@/renderer/features/sessions/unread-response-store";
import { useTerminalHeight } from "@/renderer/features/terminal/terminal-store";
import { useTerminalRegistry } from "@/renderer/features/terminal/use-terminal-registry";
import { useWorkspaceTabs } from "./use-workspace-tabs";
import {
  useWorkingTree,
  repoDisplayName,
} from "@/renderer/features/git/use-working-tree";
import { useChatSession } from "@/renderer/features/chat/session/use-chat-session";
import { isChatStarted } from "@/renderer/features/chat/session/chat-driver-store";
import {
...
} from "@/renderer/features/chat/session/auto-continue-watcher";
import {
  getProjectTabs,
  openProjectTab,
  closeProjectTab,
...
import { cachedPrTitle } from "@/renderer/features/pr/pr-store";
import {
  forgetSession,
  rekeySession,
...
import { mergeSession } from "@/renderer/features/chat/transcript/merge-session";
import {
  fetchTranscript,
  dropTranscript,
} from "@/renderer/features/chat/transcript/transcript-sync";
import { bumpWorktreeRevision } from "@/renderer/features/worktrees/worktree-revision";
import {
  getCachedSessions,
  setCachedSessions,
  getCachedTranscripts,
  setCachedTranscripts,
} from "@/renderer/features/sessions/session-cache";
...
import {
  ...
  isNewSession,
  forgetNewSession,
} from "@/renderer/features/sessions/new-session-ids";
...
import type { SessionListItem } from "@/renderer/features/sessions/session-list";
import type {
  FileEntry,
  RepoFileGroup,
...
function ProjectWorkspaceImpl({
  ...
  // A component identity per project, so switching projects remounts the editor
  // against the right file and skill index instead of leaking the old one.
  const commentInput = useMemo(
    () => makeCommentInput(project.encoded),
    [project.encoded],
  );
  ...
  // will go to — same start a click on the inactive composer does, just earlier,
  // so `claude` is booting while you keep commenting. The target is the MRU chat
  // tab because comments are mostly made on diff/file/PR tabs, where there is no
  // selected session; with no chat tab open there is nothing to start.
  const commentCountRef = useRef(totalComments);
  useEffect(() => {
    const grew = totalComments > commentCountRef.current;
    commentCountRef.current = totalComments;
    if (!grew || !mruChatSessionId) return;
    if (isChatStarted(chatTerminalId(project.encoded, mruChatSessionId)))
      return;
    void startChatFor(mruChatSessionId);
  }, [totalComments, mruChatSessionId, project.encoded, startChatFor]);
