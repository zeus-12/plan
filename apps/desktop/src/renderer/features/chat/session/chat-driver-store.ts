...
import type { ChatEngineId, StartChatResult } from "@/common/chat-engines";
...
/** Whether `chatId` has a driver right now, read outside React. */
export function isChatStarted(chatId: string | null): boolean {
  return chatId ? started.has(chatId) : false;
}
