...
export default function App() {
  ...
  const handleAddVersion = useCallback(
    (text: string) => {
      const currentLength = versionsRef.current.length;
      if (currentLength > 0) {
        setCompareBase(currentLength - 1);
      }
      addVersion(text);
    },
    [addVersion]
  ...
  // Pull init data from main process
  useEffect(() => {
    if (!window.electronAPI) return;
    window.electronAPI.getInit().then((data) => {
      if (data) {
        setFile(data.file);
        if (data.versions.length > 0) {
          initVersions(data.versions);
          setCompareBase(data.versions.length > 1 ? data.versions.length - 2 : 0);
        }
        if (data.pendingContent) {
          addVersion(data.pendingContent);
          if (data.versions.length > 0) {
            setCompareBase(data.versions.length - 1);
          }
        }
      }
    });
  }, [initVersions, addVersion]);

  // Listen for file changes from watcher
  useEffect(() => {
    if (!window.electronAPI) return;
    const cleanup = window.electronAPI.onAutoContent((content: string) => {
      handleAddVersion(content);
    });
    return cleanup;
  }, [handleAddVersion]);

  // Persist session on version changes
  useEffect(() => {
    if (!file || !window.electronAPI || versions.length === 0) return;
    window.electronAPI.saveSession(file, versions);
  }, [file, versions]);

  const fileName = file ? file.replace(/^.*[/\\]/, "") : null;

  return (
    <div className="mx-auto min-h-screen max-w-[1800px] px-6 py-8" style={{ paddingTop: 40 }}>
      {/* Header — draggable title bar region */}
      <header className="mb-8 flex items-center justify-between" style={{ WebkitAppRegion: "drag" } as React.CSSProperties}>
      ...
      {/* Empty state — no file associated, paste fallback */}
      {!latest && !file && (
        <div className="mt-24 flex flex-col items-center">
          <p
            className="mb-1 font-[family-name:var(--font-mono)] text-sm"
      ...
      {latest && (
        <div className="space-y-5">
          ...
          {/* Manual paste input — only when no file is associated */}
          {!file && (
